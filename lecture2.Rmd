...
You understand __Difference between R and RStudio__.
<br>



You __know the difference between writing code in console and a file__
<br>



You know __how to execute basic arithmetic operation and conditions.__
<br>


You know __how to access values from a object.__
<br>
...
Go to the project `Your Turn - Lecture 2`

Open a file named `exercise for lecture 2.Rmd`
...
A function is sequence of commands.

A functions requires inputs and using the given inputs, it provides output.
...
.panel[.panel-name[Address of a file]

The location of a file is referred to as address

In order to import data to R from computer or in order to export data from R o computer we need the exact location of where there file has to be read from or saved to.

For example: 

The slides you are looking at now are store in my computer at

>D:/R projects/Fundatmentals-of-Data-Analysis-Using-R/lecture2.Rmd

<br>

]

.panel[.panel-name[Importing a csv]

function: `read_csv()`

required input: address of the file

```{r}
read_csv(file = "D:/R projects/Fundatmentals-of-Data-Analysis-Using-R/data/survey_response.csv") -> data_survey
```

```{r include=FALSE}
c(
  "time_response","user_name","age_years",
           "gender","hh_num","height_cm","weight_kg",
           "prefrence_entertainment","play_mobilegame",
           "mobile_game_addict", "price_mobile",
           "brand_mobile","recharge_internet",
           "e_vaccine_hh_num", "satisfaction_cg",
           "activity_pandemic_over", "prefer_online_class",
           "preference_food_type", "preference_dish",
           "budget_dinner") -> colnames(data_survey)

data_survey[,-c(2)] -> data_survey
```
...
.panel[.panel-name[Exporting as csv]

```{r}
data_survey[c(1:25),c(5,7,12)] -> data_subset
```


```{r}
write_csv(data_subset, 
          "D:/R projects/Fundatmentals-of-Data-Analysis-Using-R/data/survey_response_subset.csv")
```
