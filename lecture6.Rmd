---
title: "Implementation of basic statistical tools in R"
subtitle: "Lecture 6"
author: "Ayush Patel"
...
background-image: url(https://images.metmuseum.org/CRDImages/is/original/DP234080.jpg)
background-size: contain


# Implementation of basic statistical tools in R
...
.pull-left[
[Background image:  Black Stork in a Landscape](https://images.metmuseum.org/CRDImages/is/original/DP234080.jpg)
]
...
You understand __You are familiar with fundamental tools for statistical analysis__.
<br>
...
.panelset[
.panel[.panel-name[The Experiment]

## [The Experiment from Learning Statistics with R](https://learningstatisticswithr.com/)
.right[by Danielle Navarro]

.center[
<iframe src="https://giphy.com/embed/QU1pSfyEynvgY" width="480" height="258" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href="https://giphy.com/gifs/QU1pSfyEynvgY">via GIPHY</a></p>
]

]
.panel[.panel-name[Some notations]

#### _Size Parameter_ (N) -Number of trials

#### _Success Probability_ ( $\theta$ ) - Success Probability

#### _Result of Experiment_ (X) - Random Variable
...
.panel[.panel-name[Probability of Success (point)]
...
```{r}
dbinom(x = c(0,1,2,3,4), size = 20, prob = 0.167)
```

```{r}
sum(dbinom(x = c(0,1,2,3,4), size = 20, prob = 0.167))
```

]

.panel[.panel-name[Probability of Success (cumulative)]
...
```{r}
pbinom(q = 4,size = 20,prob = 0.167)
```


]

.panel[.panel-name[Generate Random Variables]

#### Generate random numbers from Binomial Distribution

```{r}
rbinom(n = 1, size = 20,prob = 0.167)
```

```{r}
rbinom(n = 10, size = 20,prob = 0.167)
```

]

.panel[.panel-name[Distribution Plot]

```{r include=FALSE} 

dbinom(x = c(0:20),size = 20,prob = 0.167) -> pr_val

```

```{r echo=FALSE, fig.align='center'}
tibble(
  Number_of_success = c(0:20),
  probability_of_success = pr_val
) %>% 
  ggplot(aes(Number_of_success,
             probability_of_success))+
  geom_col(fill = "steelblue", alpha = 0.5)+
  labs(
    title = "Binomial Distribution",
    subtitle = "success probability = 0.167, number of trials = 20",
    x = "Number of success",
    y = "Probability"
  )+
  theme_bw()
...
Go to the project `Your Turn - Lecture 6`

Open a file named `exercise for lecture 6.Rmd`
...
.panel[.panel-name[The Distribution]

## Two Parameters

#### Mean ( $\mu$ )

#### Standarad Deveiation ( $\sigma$ )

## An important characteristic

68.3% of area within  $\pm$ 1 $\sigma$  <br>
95.4% of area within  $\pm$ 2 $\sigma$ <br>
99.7% of area within  $\pm$ 3 $\sigma$ <br>

]

.panel[.panel-name[How it looks]


```{r echo=FALSE, fig.align='center'}
tibble(
  random_var = c(rnorm(100000,0,1))
) %>% 
  ggplot()+
  geom_density(aes(random_var))+
  labs(
    title = "The Standard Normal Distribution",
    subtitle = "sd = 1, mean = 0"
  )+
  theme_bw()
```
...
```{r echo=FALSE, fig.align='center'}
tibble(
  random_var = c(rnorm(10000,5,1), rnorm(10000,7,1)),
  mean_val = c(rep("mean = 5",10000),
               rep("mean = 7",10000))
) %>% 
  ggplot()+
  geom_density(aes(random_var, fill = mean_val),
               alpha = 0.4)+
  labs(
    title = "Normal Distributions with same Standard deviations",
    subtitle = "sd = 1"
  )+
  theme_bw()
```
...
```{r echo=FALSE, fig.align='center'}
tibble(
  random_var = c(rnorm(10000,5,3), rnorm(10000,5,1)),
  mean_val = c(rep("sd = 3",10000),
               rep("sd = 1",10000))
) %>% 
  ggplot()+
  geom_density(aes(random_var, fill = mean_val),
               alpha = 0.4)+
  labs(
    title = "Normal Distributions with same Mean",
    subtitle = "Mean = 5"
  )+
  theme_bw()
```
...
.panel[.panel-name[Understanding the characteristic]

.center[Assume a normal distribution with mean 5 and standard deviation 1.5]
...
```{r}
pnorm(q = 3.5,mean = 5,sd = 1.5)
pnorm(q = 6.5,mean = 5,sd = 1.5)

pnorm(q = 6.5,mean = 5,sd = 1.5) - pnorm(q = 3.5,mean = 5,sd = 1.5)
```
...
# The goodness of fit test (Chi-square)
...
```{r}
table(data_survey$prefrence_food)
```


]

.panel[.panel-name[My Hypothesis]

My hypothesis is that the section of preference of food is not random and does not have equal probability of occurrence in data.

So, I create a NULL Hypothesis that the probability of choosing food preference in 0.25 for all 4 choices.

]

.panel[.panel-name[GoF statistic]

```{r}
table(data_survey$prefrence_food) -> observed

expected <- c(59.25,59.25,59.25,59.25)
...
.panel[.panel-name[Big ebough to reject NULL?]

#### Allow for 5% chance of TYPE 1 error.

```{r}
qchisq(p = 0.95,df = 3)
```

Clear the value of GoF we got is more than 7.81. We can reject NULL.

```{r}
goodnessOfFitTest(as.factor(data_survey$prefrence_food),
...
```{r}
head(data_survey$height_cm)
```

```{r include=FALSE}
se.est <- sd(data_survey$height_cm)
```
